...
void config_3430sdram_ddr (void)
{
	...
	for (i = 0; i < sizeof (sdram_timings) / sizeof (sdram_timings[0]); ++i) {
		...
		if (sdrc_probe (&sdram_timings[i], 0x80000000)) {
			dram_name = sdram_timings[i].name;
			return;
		}
	}
	dram_name = "Unknown!!!";
...
#elif defined(CFG_3430SDRAM_DDR)
/*********************************************************************
...
#define MUX_DEFAULT()\
	...
	MUX_VAL(CP(MMC2_DAT1),      (IDIS | PTD | DIS | M4)) /*MMC2_DAT1 - LED 2 */ \
    ...
    MUX_VAL(CP(JTAG_EMU0),      (IDIS | PTD | DIS | M4)) /*JTAG_EMU0 - LED 1 */	\
...
#define DEBUG_LED1			133	/* gpio */
#define DEBUG_LED2			11	/* gpio */
...
void blinkLEDs (int forever)
{
	...
	p1 = (unsigned long *)OMAP34XX_GPIO1_BASE;
	p5 = (unsigned long *)OMAP34XX_GPIO5_BASE;
	if (!forever) {
		for (i = 0; i < 50000; i++) {
			/* turn LED1 on and LED2 off */
			*(unsigned long *)(p5 + 0x94) = 1 << (DEBUG_LED1 % 32);
			*(unsigned long *)(p1 + 0x90) = 1 << (DEBUG_LED2 % 32);

			/* delay for a while */
			delay (5000);

			/* turn LED1 off and LED2 on */
			*(unsigned long *)(p5 + 0x90) = 1 << (DEBUG_LED1 % 32);
			*(unsigned long *)(p1 + 0x94) = 1 << (DEBUG_LED2 % 32);
